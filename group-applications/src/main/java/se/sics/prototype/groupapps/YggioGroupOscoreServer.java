			...
			if (isConfirmable || replyToNonConfirmable) {
				...
				if (serverName.equals("Server1") || 
 				    ...
 				    serverName.equals("Server3")) {
						double temperatureC = IndoorSimulator.simulateTemperatureCelsius(serverName);
						int co2 = IndoorSimulator.simulateCO2ppm(serverName);
 					        json.addProperty("temperature", temperatureC);
						json.addProperty("co2", co2);
				} else {
						double humidity = IndoorSimulator.simulateHumidity(serverName);
						double airPressure = IndoorSimulator.simulatePressure(serverName);
